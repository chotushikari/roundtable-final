...
`GRADIUM_API_KEY`, Gradium voice IDs, and `SARVAM_API_KEY` are server-only TTS credentials. They must never be added to client-visible environment variables, logs, API responses, examples with a real value, or source control. The Gradium voice is selected only from server environment settings. `/api/ai/sarvam/tts` accepts only the same bearer credential from Agora GenericTTS, so it cannot become a public paid-synthesis proxy; it fixes the server-owned Bulbul v3 model and `shubh` speaker instead of trusting request-supplied provider settings.

The local `DigitalPanelStage` bundles an original AI-host visual and renders only role and voice-agent state already available in the browser. `PROTOFACE_API_KEY` is server-only; Agora `GenericAvatar` receives it in the managed-agent request and the browser receives only the resulting remote RTC video track. The visual layer never receives candidate microphone or camera media. Emotion/personality/deception inference and hiring evidence from this presentation are prohibited.
