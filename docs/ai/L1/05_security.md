...
`GRADIUM_API_KEY` and Gradium voice IDs are server-only TTS credentials. They must never be added to client-visible environment variables, logs, API responses, examples with a real value, or source control. The Gradium voice is selected only from server environment settings. The legacy `/api/ai/sarvam/tts` route is not selected by live agents.

`RoundTablePanel` renders only bundled portrait assets plus role and voice-agent state already available in the browser. The visual layer never receives candidate microphone or camera media, starts no external avatar provider, and has no assessment input. Emotion/personality/deception inference and hiring evidence from this presentation are prohibited.
