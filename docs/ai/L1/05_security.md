...
`SIMLI_API_KEY` is server-only. The candidate-authenticated avatar route may return only a short-lived visual-session token and ICE configuration, never the account key. It forwards only the AI agent's remote audio track; candidate microphone/camera media, emotion/personality/deception inference, and hiring evidence are prohibited. Avatar failure must leave the local role presentation usable.
