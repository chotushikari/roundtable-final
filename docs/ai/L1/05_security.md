...
The local `DigitalPanelStage` bundles an original AI-host visual and renders only role and voice-agent state already available in the browser. It makes no third-party visual-media request and receives no candidate microphone or camera media. Emotion/personality/deception inference and hiring evidence from this presentation are prohibited.
