...
- `components/RoundTablePanel.tsx`: disclosed five-person visual panel with active-role highlight, shared context, and presentation-only handoff animation.
...
- `components/CompanyDashboard.tsx`: Google-only interviewer auth, auto-provisioned private workspace, command-center dashboard, server-backed finale/adaptive interview builder, recruiter resume/link flow with reviewed Gmail/Calendar handoffs, candidate evidence review, live/completed session pipeline, and authenticated voice-to-private-link orchestration.
- `components/RecruiterVoiceControl.tsx` and `lib/recruiter-voice.ts`: opt-in Web Speech control plus conservative English/Hinglish request slot collection; they expose working state and require a separate recipient-confirmation before email delivery. Calendar delivery remains click-only.
- `components/RecruiterSignalField.tsx`: reduced-motion-safe presentation-only Three.js signal field for the recruiter command center; its visual density is derived only from already-projected candidate and completed-evidence counts.
...
- `app/api/ai/sarvam/tts`: retained legacy authenticated Sarvam PCM bridge; live interview agents do not select it.
...
- `lib/agora-server.ts`: combined tokens and managed-agent start/stop for the voice-only Gradium pipeline.
- `lib/interview-tts.ts`, `lib/public-url.ts`: required Gradium TTS selection and public Agora callback origin resolution.
