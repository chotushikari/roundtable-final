...
- Voice update: server-only `GRADIUM_API_KEY` plus a server-selected Gradium voice selects the primary stable delivery voice. Sarvam remains the native fallback and MiniMax the no-key fallback; the one-agent architecture does not perform mid-session voice swaps.
