...
- Presentation update: `DigitalPanelStage` defaults to an original bundled AI-host visual and actual voice-agent state. When `PROTOFACE_API_KEY` and `PROTOFACE_AVATAR_ID` are present, the same managed agent adds Protoface through Agora `GenericAvatar`, publishing a real video track to the same RTC channel under the reserved avatar UID. It never processes candidate media or assessment evidence; a failed/missing avatar leaves the voice interview and local visual usable.
