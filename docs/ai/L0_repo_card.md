...
- Voice update: server-only `GRADIUM_API_KEY` plus the five role-specific Gradium voice IDs enables distinct Hiring Manager, Technical, Product, Customer, and Behavioural speakers. A single listening controller and prestarted role speakers remain in the same RTC room; the server selects exactly one speaker for each reply, with no candidate reconnect during handoff.
- Startup resilience update: invitation bootstrap returns a non-persisted five-minute signed candidate start grant alongside the secure cookie, so the immediate managed-agent start cannot fail only because the browser has not committed the freshly issued cookie yet.
- Presentation update: `RoundTablePanel` uses five bundled role portraits around one shared context and only animates the server-selected active role. It creates no avatar media track and never processes candidate media or assessment evidence.
