...
- Presentation update: the optional `simli-client` sidecar receives only remote Agora agent audio, publishes a visual interviewer independently of assessment/controller state, clears its buffer on recorded agent interruptions, and safely falls back to the local role card.
