...
- Latest review: 2026-09-10 human-decision sprint — Job hiring bars are frozen into immutable blueprints and panel handoffs prioritize shared evidence gaps. Recruiters can now record append-only `advance`, `hold`, `decline`, or `needs_review` decisions with a rationale; these are separate from AI assessment, attributed to the recruiter, and audit logged.
