...
- Latest review: 2026-09-10 interview-builder sprint — Job hiring bars are frozen into immutable blueprints and panel handoffs prioritize shared evidence gaps. The recruiter builder now creates either the five-role, ten-minute finale showcase or a configurable two-to-five-role adaptive interview with a 5–90 minute budget; the selected settings are server-validated and persisted. Recruiters can record append-only `advance`, `hold`, `decline`, or `needs_review` decisions with a rationale; these are separate from AI assessment, attributed to the recruiter, and audit logged.
