...
The company dashboard has two functional blueprint modes. `demoMode: true` creates the finale showcase: one project is explored by all five panel roles in ten minutes. Adaptive interviews let a recruiter choose two to five perspectives and a 5–90 minute budget. Both settings are persisted in the definition and copied into the immutable published version; the browser never selects speakers or scores. The showcase's hiring-manager greeting combines introduction and background into the first question. Five substantive answers complete the panel; the agent gives a short closing statement, then the browser finalizes after the closing transcript and end of agent speech. The panel guarantees one answered turn per role, but does not force a fixed speaking order: evidence gaps can hand the next turn to the appropriate perspective (for example, Product after technically sound work with no customer impact). Allow 7–9 minutes for new ten-minute dashboard demos including coding and design; older invitations retain their duration. The demo waits 1.5 seconds of silence before responding. Candidate speech fragments are grouped under the pending question, and an introduction alone does not complete the project question. A completed question transcript plus end-of-speech state produces a candidate transport receipt; an unheard/interrupted question is repeated before that role can advance. Explicit “skip”, “I don't know”, or “that's my answer” allows a short answer to finish.
