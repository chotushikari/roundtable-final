...
The company dashboard has two functional blueprint modes. `demoMode: true` creates the finale showcase: one project is explored in a fixed ten-minute sequence—Hiring Manager, Technical, Product, Customer, then Behavioural. Adaptive interviews let a recruiter choose two to five perspectives and a 5–90 minute budget. Both settings are persisted in the definition and copied into the immutable published version; the browser never selects speakers or scores. The showcase gives one substantive answer to every role, then a short closing statement before finalization. Evidence adapts each role's question, never its sequence. A single fixed fifteen-second candidate countdown runs before the agent starts, so it does not consume showcase time. It previews the concise panel welcome; the actual first turn welcomes the candidate and asks the first question. The demo waits 1.5 seconds of silence before responding. Candidate speech fragments are grouped under the pending question, and an introduction alone does not complete the project question. A completed question transcript plus end-of-speech state produces a candidate transport receipt; an unheard/interrupted question is repeated before that role can advance. Explicit “skip”, “I don't know”, or “that's my answer” allows a short answer to finish.

The recruiter workflow makes the required sequence visible: set at least three competencies, build a blueprint, add candidates, then review completed evidence. Its quick-action buttons navigate to the corresponding live workflow; role templates, seniority, and selected focus chips populate the server-backed blueprint inputs. Once a private invitation is generated, a recruiter can explicitly connect Google delivery, then use **Send invitation** or **Send calendar invite**. The browser uses a short-lived Google OAuth provider token with `gmail.send` and `calendar.events` scopes and sends directly from that recruiter’s account; RoundTable never stores the token. Reviewable Gmail and Calendar handoffs remain available. These actions are recruiter-initiated and do not create recruiter-visible live scores or automatic decisions.
...
The workspace has one **Canvas**: an embedded Excalidraw surface with selection, shapes, arrows, lines, drawing, erase, and text tools. It autosaves into the private session artifact; **Share checkpoint** remains optional for durable assessment evidence. Candidates can ask for feedback in natural language, open a workspace, ask for a hint, or explain their approach; the application understands the active task and evaluates only saved work. The interviewer checks expected code logic and saved canvas labels/arrows without running candidate code or claiming live screen access. A complete code task or Client/API Server/Database canvas diagram with at least two arrows gives grounded feedback and hands off automatically to the next panel role.
...
- Google OAuth must request and be approved for `https://www.googleapis.com/auth/gmail.send` and `https://www.googleapis.com/auth/calendar.events` when direct recruiter delivery is enabled
...
3. Publishing freezes an immutable version and returns a seven-day, single-use candidate URL. The dashboard displays it, copies it after **Copy link**, or opens a personalised Gmail draft/Google Calendar hold after explicit recruiter action. Candidate details and the optional plain-text resume are attached here, never on the candidate consent page.
...
5. Demo mode begins with a brief Hiring Manager prompt, then proceeds in a fixed judge-friendly order: Hiring Manager, Technical, Product, Customer, and Behavioural. It collects one substantive answer per role; shared evidence adapts each role's question, never the order. Product tests customer impact and Customer runs a spoken role-play. Pause, repeat, and brief readiness requests do not advance or score the interview. The candidate sees a plain-language Panel focus explanation but never a score. Candidate progress reports answered roles, not merely generated questions. The deadline starts after agent startup completes.
...
In the recruiter Candidates tab, a human records an `advance`, `hold`, `decline`, or `needs review` decision with a required rationale. These append-only records are separate from the AI assessment, are attributed to the authenticated recruiter, and write an audit-log entry. The AI never creates a human decision.
...
Candidate interviews show an accessible preparation screen while the browser joins RTC and waits for the actual AI panel. It provides AI disclosure, voice guidance, and evidence boundaries for at least 15 seconds, then opens only after both the RTC client and panel are connected. A slow connection remains visibly labeled as setup—not interview time—and provides an exit path.
...
New dashboard defaults target a Software Engineer Intern with zero experience. Choose Python, JavaScript, or TypeScript in the editor; selected language is autosaved while you work. Ask for feedback in natural language and the interviewer reads the latest autosaved snapshot without requiring a checkpoint click. A candidate may leave an unfinished workspace task with “next please”; up to three attempts are retained as neutral workspace activity, then the panel moves on without asserting completion.

The candidate-facing demo does not execute code or mention E2B. It reviews the saved code structure and expected requirements, then automatically moves to the next role when the workspace task is complete. Natural workspace explanations and leave-task requests are stored as bounded workspace events, never as candidate transcript evidence or assessment findings. Incomplete work appears as a neutral human-review status in the report. E2B remains a server-scoped, capped tool for a future explicitly enabled exercise; it has no application secrets.
