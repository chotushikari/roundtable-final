...
The company dashboard has two functional blueprint modes. `demoMode: true` creates the finale showcase: one project is explored in a fixed ten-minute sequence—Hiring Manager, Technical, Product, Customer, then Behavioural. Adaptive interviews let a recruiter choose two to five perspectives and a 5–90 minute budget. Both settings are persisted in the definition and copied into the immutable published version; the browser never selects speakers or scores. The showcase gives one substantive answer to every role, then a short closing statement before finalization. Evidence adapts each role's question, never its sequence. A fixed twenty-second candidate preparation period runs before the agent starts, so it does not consume showcase time. The demo waits 1.5 seconds of silence before responding. Candidate speech fragments are grouped under the pending question, and an introduction alone does not complete the project question. A completed question transcript plus end-of-speech state produces a candidate transport receipt; an unheard/interrupted question is repeated before that role can advance. Explicit “skip”, “I don't know”, or “that's my answer” allows a short answer to finish.
...
Candidate interviews show an accessible preparation screen while the browser joins RTC and waits for the actual AI panel. It provides AI disclosure, voice guidance, and evidence boundaries for up to 20 seconds, then opens only after both the RTC client and panel are connected. A slow connection remains visibly labeled as setup—not interview time—and provides an exit path.
