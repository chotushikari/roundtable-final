...
The recruiter workflow makes the required sequence visible: set at least three competencies, build a blueprint, add candidates, then review completed evidence. Its quick-action buttons navigate to the corresponding live workflow; role templates, seniority, and selected focus chips populate the server-backed blueprint inputs. The **Voice command** control accepts English or Hinglish and collects the role, seniority, candidate name, and candidate email over one or more utterances. Once complete, it uses the same authenticated job, blueprint, candidate, and publication APIs to create a private interview link, then shows an **Interview created** notification. Candidate emails remain editable before delivery. The recruiter may say “send invitation”; RoundTable reads the recipient back and requires “confirm send” before using the connected Google delivery account. Without a connected account it asks the recruiter to connect Google delivery or use the reviewable Gmail draft. The browser uses a short-lived Google OAuth provider token with `gmail.send` and `calendar.events` scopes and sends directly from that recruiter’s account; RoundTable never stores the token. Reviewable Gmail and Calendar handoffs remain available. These actions are recruiter-initiated and do not create recruiter-visible live scores or automatic decisions.
