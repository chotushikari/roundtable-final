...
The company dashboard has two functional blueprint modes. `demoMode: true` creates the finale showcase: one project is explored in a fixed ten-minute sequence—Hiring Manager, Technical, Product, Customer, then Behavioural. Adaptive interviews let a recruiter choose two to five perspectives and a 5–90 minute budget. Both settings are persisted in the definition and copied into the immutable published version; the browser never selects speakers or scores. The showcase gives one substantive answer to every role, then a short closing statement before finalization. Evidence adapts each role's question, never its sequence. A fixed fifteen-second candidate preparation period runs before the agent starts, so it does not consume showcase time. The panel's opening is the first interview question, not a generic spoken invitation. The demo waits 1.5 seconds of silence before responding. Candidate speech fragments are grouped under the pending question, and an introduction alone does not complete the project question. A completed question transcript plus end-of-speech state produces a candidate transport receipt; an unheard/interrupted question is repeated before that role can advance. Explicit “skip”, “I don't know”, or “that's my answer” allows a short answer to finish.

The recruiter workflow makes the required sequence visible: set at least three competencies, build a blueprint, add candidates, then review completed evidence. Its quick-action buttons navigate to the corresponding live workflow; role templates, seniority, and selected focus chips populate the server-backed blueprint inputs. Once a private invitation is generated, the recruiter can open a personalised Gmail draft or a Google Calendar event prefilled with the candidate, recruiter, interview details, and signed link. These browser handoffs use the recruiter's existing Google sign-in and require review/send or save by that recruiter; RoundTable never silently sends email or creates events. They do not create recruiter-visible live scores or automatic decisions.
...
- `APP_BASE_URL`
- `SARVAM_API_KEY` (optional, server-only; enables the authenticated Sarvam Bulbul v3/`shubh` PCM bridge for interview and homepage-demo TTS, otherwise MiniMax remains the startup fallback)
- `PROTOFACE_API_KEY` and `PROTOFACE_AVATAR_ID` (optional, server-only; enables a real Protoface talking-avatar video publisher through Agora Generic Avatar, otherwise the local disclosed host visual remains)
