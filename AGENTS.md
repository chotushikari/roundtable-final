...
- Presentation: `DigitalPanelStage` is a local, role-aware AI panel host driven by server-owned active-role and agent-state data. It is deliberately disclosed as an AI visual host rather than claimed live video or lip-sync; it receives no candidate media, changes no controller state, and supplies no assessment evidence.
