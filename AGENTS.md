...
- Presentation: `RoundTablePanel` renders five disclosed static interviewer portraits around a shared context, driven only by server-owned active-role and agent-state data. It creates no RTC media track, does not receive candidate media, and cannot change controller state or assessment evidence.
