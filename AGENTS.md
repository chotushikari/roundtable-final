...
- Candidate room entry: a bounded fifteen-second preparation surface may cover cold RTC/panel startup, but it must show only truthful setup state and exit only after RTC plus the panel remote user are connected. It is not interview time, must include AI disclosure, and must leave the candidate an exit path if setup remains pending.
