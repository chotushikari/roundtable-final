...
- Candidate room entry: a bounded fifteen-second preparation surface may cover cold RTC/panel startup and must show only truthful setup state. It exits at the end of that fixed period; if the panel is still joining, the interview surface retains its real connecting state rather than fabricating readiness. The hidden room may pre-authorize and warm the optional visual-avatar transport after eight seconds, but must not start the managed agent or candidate microphone before the full preparation period. It is not interview time and must include AI disclosure.
