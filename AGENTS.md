...
- Presentation: a browser-rendered role-aware AI avatar follows the server-owned active panel role and agent state. With server-only `SIMLI_API_KEY` and `SIMLI_FACE_ID`, an optional Simli visual sidecar consumes only the remote AI audio track and renders presentation video; it never receives candidate media, changes controller state, or supplies assessment evidence. The local role card remains the fallback.
