...
- Presentation: `DigitalPanelStage` is a role-aware AI panel host driven by server-owned active-role and agent-state data. With server-only `PROTOFACE_API_KEY` and `PROTOFACE_AVATAR_ID`, the managed Agora agent attaches Protoface through Agora's `GenericAvatar` and publishes a real talking-avatar video track into the same RTC channel under `DEFAULT_AVATAR_UID`; without both credentials, the bundled disclosed host visual remains the fallback. Neither surface receives candidate media, changes controller state, or supplies assessment evidence.
