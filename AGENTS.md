...
- Voice pipeline: Agora-managed Deepgram STT plus an authenticated RoundTable custom LLM/controller endpoint; with server-only `GRADIUM_API_KEY` and a server-selected voice, Gradium is the primary stable delivery voice. Sarvam (default `anushka`) and MiniMax remain fallbacks. One physical agent keeps the selected voice for the entire session; do not claim an automatic mid-session provider fallback.
...
- Presentation: `DigitalPanelStage` is a role-aware AI panel host driven by server-owned active-role and agent-state data. Protoface is an optional experimental enhancement and attaches only when `ENABLE_PROTOFACE_AVATAR=true` plus server-only credentials are present; it publishes a real talking-avatar video track into the same RTC channel under `DEFAULT_AVATAR_UID`. Otherwise the bundled disclosed host visual remains the fallback. Neither surface receives candidate media, changes controller state, or supplies assessment evidence.
...
- Candidate room entry: a bounded fifteen-second preparation surface may cover cold RTC/panel startup, but it must show only truthful setup state and exit only after RTC plus the panel remote user are connected. The hidden room may pre-authorize and warm the optional visual-avatar transport after eight seconds, but must not start the managed agent or candidate microphone before the full preparation period. It is not interview time, must include AI disclosure, and must leave the candidate an exit path if setup remains pending.
...
- New dashboard blueprints offer an explicit `demoMode` finale showcase (all five roles, fixed ten-minute maximum) or a normal adaptive interview (recruiter-selected two-to-five roles and 5–90 minute budget). Both selections are server-validated, stored with the definition, and frozen with the published version. Showcase order is strict: Hiring Manager, Technical, Product, Customer, then Behavioural. Evidence may adapt each role's question but never its position. A fixed fifteen-second preparation period precedes agent start and candidate microphone publication, so it is not interview time. The single preparation surface previews the concise welcome that starts the first question: “Welcome to RoundTable. I'm your AI Hiring Manager.” Finish after five substantive answers and the spoken closing; pause/repeat/readiness controls must not count. Candidate progress counts answered roles, not generated questions. Existing invitations retain their duration and sequence; normal interviews retain adaptive priorities. Demos with at least ten minutes assign code to Technical and canvas to Product. Default to intern tasks accepting coursework. Save selected Python/JavaScript/TypeScript language in checkpoints. Workspace acknowledgements describe only saved artifacts; E2B reports selected test-case outcomes, not a blanket correctness claim.
