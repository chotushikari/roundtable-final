...
- Voice pipeline: Agora-managed Deepgram STT plus an authenticated RoundTable custom LLM/controller endpoint; with server-only `SARVAM_API_KEY`, Agora's native Sarvam TTS adapter uses a supported delivery voice (default `anushka`), otherwise MiniMax is the startup fallback. Do not claim an automatic mid-session provider fallback.
