...
export default function ConversationComponent({
  ...
  if (!compactDemo && !companionDemo && !preparationComplete) {
    const stillConnecting = preparationSeconds >= 20 && !roomReady;
    const status = !joinSuccess
      ? 'Joining your secure voice room'
      : !isAgentConnected
        ? 'Waiting for the AI interview panel'
        : 'Your room is ready';
    return (
      <main className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-[#0d0d0d] px-5 text-[#ededed] before:pointer-events-none before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_50%_0%,rgba(62,207,142,.14),transparent_34%),linear-gradient(rgba(255,255,255,.018)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.018)_1px,transparent_1px)] before:bg-[size:auto,48px_48px,48px_48px]">
        <section className="relative z-10 w-full max-w-xl rounded-2xl border border-[#2b3930] bg-[#121513ee] p-6 shadow-[0_28px_100px_rgba(0,0,0,.42)] sm:p-8" aria-live="polite">
          <div className="flex items-center justify-between gap-4"><span className="font-mono text-[10px] font-semibold uppercase tracking-[.16em] text-[#57d592]">RoundTable AI · Interview preparation</span><span className="rounded-full border border-[#355344] bg-[#163123] px-3 py-1 font-mono text-[10px] text-[#81dfac]">{roomReady ? 'Room ready' : `${Math.max(0, 20 - preparationSeconds)}s setup`}</span></div>
