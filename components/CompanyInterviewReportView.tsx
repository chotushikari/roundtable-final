...
function EvidenceMap({ report }: { report: CompanyInterviewReport }) {
  const signals = report.competencies.slice(0, 5);
  const coverage = report.competencies.length ? Math.round((report.coverage.covered.length / report.competencies.length) * 100) : 0;
  ...
  return <article className="rounded-xl border border-[#2a2a2a] bg-[#151515] p-5">
    <div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-semibold">Evidence map</h3><p className="mt-1 text-xs text-[#777]">Observed competency coverage and rating signals.</p></div><span className="rounded-full border border-[#3a5c48] bg-[#3ecf8e10] px-2.5 py-1 font-mono text-[10px] text-[#72dfa8]">NOT A DECISION</span></div>
    <div className="mt-5 grid items-center gap-5 sm:grid-cols-[116px_1fr]">
      <div className="relative mx-auto grid h-28 w-28 place-items-center">
        <svg viewBox="0 0 100 100" className="h-28 w-28 -rotate-90" aria-label={`${coverage}% competency evidence coverage`}><circle cx="50" cy="50" r="42" fill="none" stroke="#292929" strokeWidth="8"/><circle cx="50" cy="50" r="42" fill="none" stroke="#3ecf8e" strokeWidth="8" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset}/></svg>
        <span className="absolute text-center"><b className="block text-2xl font-medium tracking-tight">{coverage}%</b><small className="text-[9px] uppercase tracking-[.12em] text-[#747474]">covered</small></span>
      </div>
      <div className="grid gap-3">{signals.length ? signals.map((item) => { const value = item.rating === null ? 0 : item.rating / 4 * 100; return <div key={item.id}><div className="mb-1.5 flex items-center justify-between gap-3 text-xs"><span className="truncate text-[#bdbdbd]">{item.name}</span><span className="shrink-0 font-mono text-[10px] text-[#77dda9]">{item.rating === null ? '—' : `${item.rating}/4`}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-[#282828]"><i className="block h-full rounded-full bg-gradient-to-r from-[#277952] to-[#3ecf8e]" style={{ width: `${value}%` }}/></div></div>; }) : <p className="text-xs text-[#777]">No competency evidence was recorded.</p>}</div>
    </div>
  </article>;
}
