...
function EvidenceVisuals({ report }: { report: CompanyInterviewReport }) {
  const observed = report.competencies.filter((item) => item.rating !== null);
  const coverage = report.competencies.length ? Math.round(observed.length / report.competencies.length * 100) : 0;
  const panelCoverage = report.interview.panelRoles.length ? Math.round(report.coverage.rolesWithEvidence.length / report.interview.panelRoles.length * 100) : 0;
  return <article className="rounded-xl border border-[#2a2a2a] bg-[#151515] p-5">
    <div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-semibold">Evidence coverage</h3><p className="mt-1 text-xs leading-5 text-[#777]">A view of what was observed—not a candidate score.</p></div><span className="rounded-full border border-[#345443] bg-[#3ecf8e12] px-2 py-1 font-mono text-[9px] text-[#3ecf8e]">Human review</span></div>
    <div className="mt-5 grid items-center gap-5 sm:grid-cols-[7rem_1fr]"><div className="relative mx-auto grid h-28 w-28 place-items-center rounded-full" style={{ background: `conic-gradient(#3ecf8e 0 ${coverage}%, #292929 ${coverage}% 100%)` }}><div className="grid h-20 w-20 place-items-center rounded-full bg-[#151515] text-center"><strong className="text-xl">{coverage}%</strong><span className="font-mono text-[8px] uppercase tracking-[.12em] text-[#777]">observed</span></div></div><div className="grid gap-3"><MetricBar label="Competencies" value={coverage} detail={`${observed.length} of ${report.competencies.length}`} /><MetricBar label="Panel perspectives" value={panelCoverage} detail={`${report.coverage.rolesWithEvidence.length} of ${report.interview.panelRoles.length}`} /></div></div>
    {observed.length > 0 && <div className="mt-5 border-t border-[#292929] pt-4"><p className="font-mono text-[9px] uppercase tracking-[.14em] text-[#777]">Observed competency ratings</p><div className="mt-3 grid gap-2">{observed.map((item) => <MetricBar key={item.id} label={item.name} value={Math.round((item.rating ?? 0) / 4 * 100)} detail={`${item.rating} / 4`} />)}</div></div>}
  </article>;
}

function MetricBar({ label, value, detail }: { label: string; value: number; detail: string }) {
  return <div><div className="mb-1 flex items-center justify-between gap-3 text-[11px]"><span className="truncate text-[#bdbdbd]">{label}</span><span className="shrink-0 font-mono text-[#777]">{detail}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-[#292929]"><i className="block h-full rounded-full bg-[#3ecf8e]" style={{ width: `${value}%` }} /></div></div>;
}
...
export function CompanyInterviewReportView({ report, onRelease, releasePending = false }: { report: CompanyInterviewReport; onRelease: () => void; releasePending?: boolean }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-[#2b2b2b] bg-[#101010] text-[#ededed] shadow-[0_32px_100px_rgba(0,0,0,.3)]">
    <header className="border-b border-[#292929] bg-[radial-gradient(circle_at_90%_0%,rgba(62,207,142,.08),transparent_35%),#131313] px-6 py-7 lg:px-8">
      ...
      <div className="mt-7 grid grid-cols-2 gap-2 md:grid-cols-3"><div className="rounded-xl border border-[#292929] bg-[#161616] p-4"><span className="text-[10px] text-[#717171]">OBSERVED</span><strong className="mt-2 block text-2xl font-medium">{report.coverage.covered.length}<small className="text-sm text-[#606060]"> / {report.competencies.length}</small></strong></div><div className="rounded-xl border border-[#292929] bg-[#161616] p-4"><span className="text-[10px] text-[#717171]">PERSPECTIVES</span><strong className="mt-2 block text-2xl font-medium">{report.coverage.rolesWithEvidence.length}<small className="text-sm text-[#606060]"> / {report.interview.panelRoles.length}</small></strong></div><div className="rounded-xl border border-[#292929] bg-[#161616] p-4"><span className="text-[10px] text-[#717171]">DURATION</span><strong className="mt-2 block text-2xl font-medium">{duration(report.session.durationSeconds)}</strong></div></div>
    </header>
